...
\begin{frame}
\frametitle{Semi-Supervised Learning with Diffusion Models}

\begin{itemize}
\item \textbf{Diffusion Model Approach}
    \begin{itemize}
        \item Denoising diffusion probabilistic model (DDPM) framework
        \item U-Net style architecture with class and timestep conditioning
        \item Gradually denoises images from random noise to clean samples
    \end{itemize}
\item \textbf{Multi-Step Training Process}
    \begin{itemize}
        \item Initial classifier training on limited labeled data
        \item Diffusion model training with pseudo-labeled unlabeled data
        \item Synthetic data generation for augmentation
        \item Final classifier training on augmented dataset
    \end{itemize}
\item \textbf{Results}
    \begin{itemize}
        \item 81.3\% accuracy with only 500 labeled examples
        \item Outperforms standard supervised learning on limited data
        \item Generates high-quality synthetic training examples
    \end{itemize}
\end{itemize}
...
\begin{frame}
\frametitle{Diffusion Model Training and Results}

\begin{figure}
    \centering
    \includegraphics[width=0.45\textwidth]{fig/diffusion_training_loss.png}
    \includegraphics[width=0.45\textwidth]{fig/diffusion_classifier_training.png}
    \caption{Diffusion model: Training loss (left) and classifier performance (right)}
\end{figure}

\end{frame}

\begin{frame}
\frametitle{Diffusion Model Generated Samples}

\begin{figure}
    \centering
    \includegraphics[width=0.95\textwidth]{fig/diffusion_final_samples.png}
    \caption{Synthetic digits generated by the diffusion model}
\end{figure}
...
\begin{itemize}
    ...
    \item Diffusion model achieves 81.30\% with limited labeled data
    ...
    \begin{itemize}
        ...
        \item Diffusion: Best performance on limited labeled data
