...
def main():
    ...
    parser.add_argument("--model", type=str, default="all", 
                        choices=["svm", "rf", "knn", "transformer", "cnn", "mlp", "all"],
    ...
    parser.add_argument("--svm-c", type=float, default=1.0,
                        help="C parameter for SVM (default: 1.0)")
    ...
    if args.model in ["svm", "all"]:
        results["SVM"] = train_svm(X_train, y_train, X_test, y_test, C=args.svm_c, gamma=args.svm_gamma)
    
    if args.model in ["rf", "all"]:
        results["Random Forest"] = train_random_forest(X_train, y_train, X_test, y_test, n_estimators=args.n_estimators)
    
    if args.model in ["knn", "all"]:
        results["KNN"] = train_knn(X_train, y_train, X_test, y_test, n_neighbors=args.n_neighbors)
